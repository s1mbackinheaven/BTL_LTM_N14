...
import server.models.GameSession;
...
public class GameSessionManager {
    private final Map<String, GameSession> activeSessions; // các trận đấu đang được diễn ra
    private final Map<String, String> playerToSession; // userId -> sessionId
    ...
    // String sessionId = generateSessionId(challenger.getname(),
    // challenged.getname());
    ...
    // playerToSession.put(challenger.getname(), sessionId);
    // playerToSession.put(challenged.getname(), sessionId);

    // System.out.println("🎮 Tạo trận đấu mới: " + sessionId + " giữa " +
    // challenger.getname() + " vs " + challenged.getname());
    ...
    // + " | Winner: " + (session.getWinner() != null ?
    // session.getWinner().getUsername() : "null")
    ...
    // Player loser = (winner == session.getPlayer1()) ? session.getPlayer2() :
    // session.getPlayer1();
