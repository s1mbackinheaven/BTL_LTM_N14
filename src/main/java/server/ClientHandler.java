...
import com.oop.game.JAR.dto.*;
import com.oop.game.JAR.enums.*;
import com.oop.game.JAR.protocol.*;
...
public class ClientHandler implements Runnable {
    ...
    private void handlerRegisReq(RegisRequest req, ObjectOutputStream objOP) {
        ...
        if (un == null || pw == null) {
            OP(new LoginResponse(false, "Vui lòng nhập đủ thông tin"), objOP);
            return;
        }
    ...
    private void handlerLoginReq(LoginRequest req, ObjectOutputStream objOP) {
        ...
        if (status == AuthStatus.INVALID_CREDENTIALS) {
            OP(new LoginResponse(false, "Sai tài khoản hoặc mật khẩu"), objOP);
            return;
        }
        if (status == AuthStatus.DB_ERROR) {
            OP(new LoginResponse(false, "Hệ thống đang bận, vui lòng thử lại sau"), objOP);
            return;
        }

        User user = userDAO.getUserByUsername(username);
        if (user == null) {
            OP(new LoginResponse(false, "Không thể lấy thông tin tài khoản"), objOP);
            return;
        }
        ...
        if (mClient.isOnline(player)) {
            OP(new LoginResponse(false, "Người chơi đang đăng nhập ở nơi khác"), objOP);
            return;
        }
    ...
    private void handlerMoveReq(MoveRequest req, ObjectOutputStream objOP) {
        ...
        if (!currentPlayer.isMyTurn()) {
            OP(new ErrorMessage("SERVER", "NOT_YOUR_TURN", "Chưa đến lượt của bạn"), objOP);
            return;
        }
        ...
        try {
            ...
            // Lấy thông tin cả 2 người chơi
            Player player1 = game.getPlayer1();
            Player player2 = game.getPlayer2();

            // Tạo GameStateUpdate để gửi cho cả 2 người chơi
            GameStateUpdate gameStateUpdate = new GameStateUpdate("SERVER",
                    new PlayerGameStateDTO(player1.getUsername(),
                            player1.getCurrentScore(), player1.getPowerUpString(),
                            player1.isMyTurn()),
                    new PlayerGameStateDTO(player2.getUsername(),
            ...
            // Gửi cho người chơi hiện tại (đã ném)
            OP(gameStateUpdate, objOP);

            // Gửi cho đối thủ
            String opponentUsername = (currentPlayer == player1) ? player2.getUsername() : player1.getUsername();
            connectionManager.sendMessageToClient(opponentUsername, gameStateUpdate);
    ...
    private void handlerLeaderboardReq(LeaderboardRequest req, ObjectOutputStream objOP) {
        ...
        try {
            ...
            for (var i : players) {
                ...
                if (ClientManager.getInstance().isOnline(player))
                    player.setBusy(true);

                p.add(new PlayerInfoDTO(player.getUsername(), player.getElo(), player.getTotalWins(),
                        player.getTotalLosses(), player.isBusy()));
            }
